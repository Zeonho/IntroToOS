...
int main(void) {
  ...
  while (TRUE) {
	...
	if (rval == 0) {
	  ...
	  if (strcmp(req.req_command, "ADD") == 0) {
		  response.result =  req.first_operand + req.second_operand;
	  }
	  else if (strcmp(req.req_command, "SUBTRACT") == 0) {
		  response.result =  req.first_operand - req.second_operand;
	  }
	  else if (strcmp(req.req_command, "MULTIPLY") == 0) {
		  response.result =  req.first_operand * req.second_operand;
	  }
	  else if (strcmp(req.req_command, "DIVIDE") == 0) {
		  response.result =  req.first_operand / req.second_operand;
	  }

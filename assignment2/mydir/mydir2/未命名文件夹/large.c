...
int dirStat(char *dirpath, struct stat statbuf,char fileArray[6][NAMESIZE]){
	DIR *dp;
	...
	while( (direntry = readdir(dp)) != NULL )
	{
		...
		// Locating the problem, after reading a directory, the S_ISDIR will fail to validate
		// Solved, change the dir back
		if (S_ISDIR(statbuf.st_mode))
		{
			if (strcmp(direntry->d_name, ".") != 0 && strcmp(direntry->d_name, "..") != 0 ){
				...
				dirStat(sub_dirpath,sub_statbuf, fileArray);
				chdir(dirpath);
			}
		}
		else{
			...
			if ( (int) statbuf.st_size > atoi(fileArray[1]) ){
...
int main(int argc, char *argv[])
{
	...
	if(choice == 1){
		...
		dirStat(dirpath, statbuf,fileArray);
